...
function stubDeps() {
  ...
  return {
    ...
    //
    // **여기를 "모르는 id 는 null" 로 엄격하게 고치지 말 것.** 그러면 탈출
    // 테스트가 조용히 무력화된다 — 탈출 경로는 알려진 퍼즐 id 가 아니므로
    // 리졸버에서 먼저 404 가 나고, safeContentPath 를 통째로 지워도 200 이
    // 되지 않는다. 가드를 검사하던 테스트가 가드를 거치지도 않게 된다.
    // 항등이어야 요청이 파일 경로 조립까지 도달한다.
...
> **이 계약은 상태 코드만으로는 검증되지 않는다.** 순서를 뒤집어도 "서명 없는 실재 퍼즐 → 403", "유효 서명 + 없는 퍼즐 → 404" 가 그대로 나온다.
>
> **404 가 나오는 자리가 둘이라는 점이 핵심이다** — `resolvePuzzleId` 가 null 을 주는 경우와 파일이 없는 경우다. 스텁 리졸버는 항등이므로 Plan 3 에서 없는 퍼즐의 404 는 **전부 파일 존재 확인에서** 나온다. 그래서 "서명이 틀리면서 퍼즐도 없는" 입력은 검증을 리졸버 뒤로 옮기는 것만으로는 깨지지 않는다 — 리졸버를 통과한 뒤 검증이 먼저 실패해 여전히 403 이다.
>
> 그래서 검사를 둘로 나눈다. `resolvePuzzleId` 호출 기록은 **리졸버보다 먼저인지**를, 위조 서명 + 없는 퍼즐은 **파일 확인보다 먼저인지**를 본다. 하나로는 둘 다 못 잡는다.
>
...
- `인덱스를 바꾸면 403 이다`
- `서명이 틀리면 없는 퍼즐이어도 403 이다` — 검증을 **파일 확인 뒤로** 옮기면 404 가 된다
- `서명이 틀리면 퍼즐 조회를 아예 하지 않는다` — 검증을 **리졸버 뒤로** 옮기면 깨진다
- `유효하게 서명된 경로 탈출도 파일에 닿지 못한다` — `safeContentPath` 를 지우면 200 이 된다
- `nickname 이 빠지면 400 이고 저장소를 때리지 않는다`
- `204 를 주고 그 세션이 실제로 폐기된다`

**변이를 넣어 확인한다면, 기대 결과가 셋 다 다르다:**

| 변이 | 깨지는 테스트 |
|---|---|
| `safeContentPath` 제거 | 탈출 테스트 1개 — 200 이 나온다 |
| 검증을 `resolvePuzzleId` **뒤로** | `퍼즐 조회를 아예 하지 않는다` **1개** |
| 검증을 **파일 존재 확인 뒤로** | 위 1개 + `없는 퍼즐이어도 403` = **2개** |

가운데 변이가 1개만 깨뜨리는 것이 맞다. 스텁 리졸버가 항등이라 없는 퍼즐도
리졸버는 통과하고, 그다음 검증이 먼저 실패해 403 이 유지된다. Plan 4 에서
`resolvePuzzleId` 가 실제 Redis 조회로 바뀌어 모르는 매치에 null 을 주기
시작하면 이 변이도 2개를 깨뜨리게 된다.
...
Task 2 에서 한 번, Task 3 에서 두 번 걸렸다.

- node-postgres 가 다중 문장 질의에 배열을 돌려준다 — 타입 정의에 없다.
- `describe.skip` 이 콜백 본문은 평가한다 — 그래서 skip 된 suite 가 외부에 접속했다.
- `createCache` 가 동기라 연결 실패를 던지지 않는다 — "연결 실패 시 부팅 실패" 가
  Redis 쪽에서는 거짓말이었다. `ping` 을 포트에 추가해 고쳤다.

셋 다 **타입은 통과했고 테스트 숫자도 정상으로 보였다.** `express` 와 `supertest` 도
같은 방식으로 틀렸을 수 있다. 타입이 통과한다고 런타임이 맞는 것은 아니다.
