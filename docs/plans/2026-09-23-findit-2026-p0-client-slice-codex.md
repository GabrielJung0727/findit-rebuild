...
> 끝낼 때마다 표시한다. Flutter 3.44 · Node 24 · npm · git · Docker 가 필요하다.
>
> **각 Task 를 끝낼 때마다 그 Task 의 diff 를 독립적으로 검토하는 단계를 넣어라.**
> Plan 4 에서 독립 검토가 17건을 잡았고 17건 모두 유효했다.
>
> **이 계획서의 Dart 코드는 실제로 돌려 봤다.** 임시 프로젝트에 풀어 `flutter analyze`
> 무경고, `flutter test` 58개 통과를 확인했다. 그렇다고 결함이 없다는 뜻은 아니다 —
> 그 과정에서 일곱 개를 고쳤고, 화면에 실제로 그려지는 위치는 테스트가 잡지 못한다.
>
> Claude Code 용 원본: [`docs/superpowers/plans/2026-09-23-findit-2026-p0-client-slice.md`](../superpowers/plans/2026-09-23-findit-2026-p0-client-slice.md) — 내용은 같고 실행 지침만 다르다. 한쪽을 고치면 다른 쪽도 고칠 것.
