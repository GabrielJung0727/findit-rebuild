...
Task 2 에서 이미 한 번 걸렸다 — node-postgres 가 다중 문장 질의에 배열을 돌려준다는 것은
타입 정의에 없고 나는 타입만 보고 코드를 썼다. `ioredis`, `express`, `supertest` 도 같은
방식으로 틀렸을 수 있다. 타입이 통과한다고 런타임이 맞는 것은 아니다.
